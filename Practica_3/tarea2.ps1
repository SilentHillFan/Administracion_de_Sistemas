...
function Opcion-Configurar {
    ...
    # 2. Forzar enlace a la interfaz y reiniciar para aplicar
    Forzar-InterfazDHCP

    # --- FORZAR DNS Y DOMINIO (Aquí está la clave) ---
    ...
    if (Validar-IP $dns) {
        ...
        # Opción 15: Nombre de Dominio (Esto fuerza a Xubuntu a aceptar el DNS)
        # Usamos un nombre genérico o podrías pedirlo, pero esto es vital.
        Set-DhcpServerv4OptionValue -ScopeId $scopeObj.ScopeId -OptionId 15 -Value "red.local" -Force | Out-Null
        
        Write-Host "DNS y Dominio configurados para forzar al cliente." -ForegroundColor Cyan
    }
