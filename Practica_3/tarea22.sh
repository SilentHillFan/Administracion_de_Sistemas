...
opcion_configurar() {
    ...
    # --- TU LÓGICA DE IPs (INTACTA) ---
    SUBNET_PRE=$(echo $IP_START | cut -d'.' -f1-3)
    SUBNET_ID="$SUBNET_PRE.0"
    SERVER_NEW_IP="$IP_START"

    INT_START=$(ip_to_int "$IP_START")
    NEXT_INT=$((INT_START + 1))
    IP_START=$(printf "%d.%d.%d.%d" \
    $(( (NEXT_INT >> 24) & 255 )) \
    $(( (NEXT_INT >> 16) & 255 )) \
    $(( (NEXT_INT >> 8) & 255 )) \
    $(( NEXT_INT & 255 )))
    ...
    # Limpieza de leases para asegurar consistencia
    ...
    # --- CONFIGURACIÓN DE FUERZA PARA DNS ---
...
EOF
    ...
    if [[ -n "$DNS_IP" ]]; then
        # Enviamos el DNS como opción obligatoria
